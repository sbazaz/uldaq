...
def main():
    ...
    try:
        ...
        configure_trigger(daqi_device, trigger_types)
        ...
        print('    Rate: ', rate, ' Hz')
        print('    Scan options:', display_scan_options(daqi_info.get_scan_options(), scan_options))
        ...
        if status.value == ScanStatus.SS_RUNNING:
...
def configure_trigger(daqi_device, trigger_types):
    # use a dummy descriptor since we are using the external digital trigger
    desc = DaqInChanDescriptor()

    # use the first available trigger type
    err = daqi_device.set_trigger(trigger_types[0], desc, 0.0, 0.0, 0)
    print('\nCalling daqi_device.set_trigger() for first trigger type')
    print('    Trigger type: ', trigger_types[0])
